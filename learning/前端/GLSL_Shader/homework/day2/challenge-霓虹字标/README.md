...
完整霓虹 hero 画面：前景 SDF 构图（圆环 + 焊接支线）、高度场光感、双层辉光、丝绸背景层、电影感配方收尾、页头与双层视差——「可以直接放进作品集的一屏」，iyO / 赛博类站点的核心画面。技术考点：多 SDF 布尔组合（含 smin）、法线光照、glow 双层、背景层混合、配方卡顺序、视差分层。

## 前置讲义

三讲的合体考试：2.1 给形状零件、2.6 给光感配方、2.8 给收尾顺序。背景丝绸的 `noise/fbm` 是骨架脚手架（2.4 不是本题前置，函数直接复用）。

## 任务清单

1. **TODO(day2-ch-1)** — fragment 前景：≥3 个 SDF 布尔组合，含一次 `smin`。建议构图：圆环（`abs(length - R) - w`）+ 斜线段 `smin` 焊一枚小圆。
2. **TODO(day2-ch-2)** — fragment：高度场法线 + 朗伯一行（讲义 2.6 三行迁移）。
3. **TODO(day2-ch-3)** — fragment：双层 glow——主玫红 w=0.08 ×0.9、次天青 w=0.12 ×0.5，指数 2.0；环体金属色填充（`0.4 + 0.6 * lit`，fwidth 抗锯齿）。
4. **TODO(day2-ch-4)** — fragment 背景：丝绸层（fbm warp、透明度 25%、流速 0.1、视差 2%）。
...
- 加分项：环上叠一次断裂（`max(sd, 断口 SDF)` 做差）；或 glow 色随呼吸微移色相
...
画面三层各管一摊：背景层管「氛围」（丝绸慢流、低对比）、前景层管「主体」（SDF 构图 + 光感 + 辉光）、收尾层管「成片感」（配方卡顺序）。层间的纪律只有一条：背景的对比度必须让给前景——25% 透明度不是美学偏好，是视觉层级的工程参数。
...
`sdCircle / sdSegment / smin`（骨架已给）· `abs(length(p) - R) - w`（圆环）· `dFdx / dFdy`（法线）· `pow(w / (abs(sd) + w), 2.0)`（glow）· 配方卡五行（2.8）。
...
float bgr = fbm(bqp * 1.3 + u_time * 0.1);
vec3 col = BG + palette(bgr * 0.8) * 0.25;    // 透明度 25%
...
float sd = min(ring, smin(seg, orb, 0.06));    // 环 + 焊接支线
...
col += ROSE * g1 * 0.9 + CYAN * g2 * 0.5;
col = mix(col, vec3(0.94, 0.80, 0.86) * (0.4 + 0.6 * lit),
