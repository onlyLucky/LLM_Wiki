...
// 丝绸的低对比色卡：玫瑰灰系（与前景主色同族不抢戏）
vec3 silkPalette(float t) {
  return vec3(0.30, 0.22, 0.26) + vec3(0.10, 0.07, 0.09) * cos(6.2831853 * (t + vec3(0.1, 0.3, 0.5)));
}
...
void main() {
  ...
  // ---- 层 0：背景丝绸（任务 4）视差 2%、流速 0.1、透明度 25% ----
  vec2 bp = p + (u_mouse - 0.5) * 0.02;
  vec2 bq = vec2(fbm(bp * 1.3 + vec2(0.0, u_time * 0.05)),
                 fbm(bp * 1.3 + vec2(5.2, 1.3)));
  float bsilk = fbm(bp * 1.3 + vec2(u_time * 0.1, u_time * 0.04) + 0.6 * bq);
  vec3 col = BG + silkPalette(bsilk) * 0.25;

  // ---- 层 1：前景构图（任务 1）视差 5% --------------------------
  vec2 fp = p + (u_mouse - 0.5) * 0.05;
  // 圆环 + 斜线段 smin 焊一枚小圆——环身 + 支线的构图
