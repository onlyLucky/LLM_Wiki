// Day 2 · 作业 challenge —— GPGPU 粒子星系（参考答案）
...
function createParticleStates(): GPUBuffer[] {
  ...
  // 初始星系盘只写进 states[0]：三条旋臂 + 高斯散布 + 切向初速度
  const init = new Float32Array(COUNT * 8);
  const gauss = () => Math.random() + Math.random() + Math.random() - 1.5;
  for (let i = 0; i < COUNT; i++) {
    // 半径幂次 0.62：外疏内密；角度 = 旋臂相位 + r × 2.2 的对数螺旋 + 高斯抖动
    ...
    init[k + 1] = gauss() * 0.045 * (1.1 - Math.min(r * 0.35, 0.7)); // y：薄盘
    init[k + 2] = Math.sin(theta) * r + gauss() * 0.05;              // z
    // 开普勒式切向初速度：近快远慢，画面从第一帧起就在转
    const vOrb = 0.5 / Math.sqrt(r + 0.12);
...
// ---- compute 管线 -------------------------------------------------
// 力场本体（噪声漂移 + 涡旋 + 鼠标引力）在 particles.wgsl 的 update 里。
