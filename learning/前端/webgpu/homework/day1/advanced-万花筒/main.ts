...
// vs 已写全；你的任务：main.ts 的 uniform 上传 + fs 侧三个函数
// toPolar → fold → mandala，每填一级，画面点亮一层。
...
const chrome = createChrome({
  ...
  title: 'KALEIDOSCOPE',
  tags: ['WEBGPU', 'WGSL'],
  hint: '完成 TODO 后：八重镜像万花筒旋转呼吸 · 鼠标驱动折数与色温',
});
...
chrome.startLoop((now) => {
  ...
  // TODO(day1-advanced-1): 把四个有效字段写进 uniforms 并上传
  //   uniforms[0] = t;
  //   uniforms[2] = mouse[0]; uniforms[3] = mouse[1];
  //   uniforms[4] = chrome.width / chrome.height;
  //   device.queue.writeBuffer(uniformBuffer, 0, uniforms);
  //   注意索引 1 是填充槽：连写 [0][1][2][3] 会把 mouse 掉进洞里
  //   （1.6 讲的原样重现——WGSL 读到的会是垃圾值，画面随时间错乱）。
