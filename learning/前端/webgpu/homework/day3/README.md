...
| 档位 | 目录 | 主题 | 前置讲义 | 净时长 |
|------|------|------|---------|--------|
| 基础 | `basic-TSL波浪网格/` | 用 TSL 复刻 Day 2 顶点波浪，代码量对比 | 3.2 | 约 1h |
...
```mermaid
flowchart TD
    A["basic：同一片海，换个引擎写<br/>节点图替换 WGSL 字符串，代码量骤减的体感课"]
...
基础档把 Day 2 demo 04 的顶点波浪原样搬来重写：那份作业是原生 WGSL 写的（main.ts 216 行 + ocean.wgsl 97 行），今天用 TSL 写出同一片海，两份代码摆在一起，引擎层的价值就不再是口号而是行数。它同时是 TSL 的最小完整闭环——positionNode、colorNode、uniform 交互一次练完。进阶档在 TSL 计算管线里重建 Day 2 的粒子状态机，再加两样新东西：文字离屏采样成吸附目标、弹簧吸附与噪声呼吸——做完它，实例清单里 iyO 首页的粒子文字对你就从魔术变成了工程。挑战档是 3.5 工作坊三个组件的全装配：玻璃材质、滚动驱动相机、bloom 后处理咬合在一页里，风格锚点 haoqi.design 与 igloo.inc，这一档的完成品可以直接当作品集片段。
...
- [basic-TSL波浪网格](./basic-TSL波浪网格/)——`TSL WAVE GRID`
