// Day 2 · Demo 06 —— GPGPU 粒子的计算着色器
// 读 src 写 dst，两组绑定组轮换（ping-pong，讲义 2.7 的状态机）。
...
struct Sim {
  mouse: vec4f,  // xyz 鼠标世界坐标（星系平面 y≈0），w 是否按下
...
// ---- simplex noise 3D（Ashima Arts / Stefan Gustavson 版，WGSL 移植）----
// 完整可抄版本；要 curl 噪声时对它做三次偏移采样再取旋度即可。
