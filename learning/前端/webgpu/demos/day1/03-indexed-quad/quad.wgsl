// Day 1 · Demo 03 —— 索引绘制的四边形
// 顶点色对角渐变，与 demo 02 的着色器结构完全相同——
// 索引改变的是「顶点怎么被取用」，片元阶段毫无感知。
...
struct VertexOut {
  ...
  @location(0) color: vec3f, // 光栅化阶段自动插值
};

@vertex
fn vs(vin: VertexIn) -> VertexOut {
  // 每个顶点只被加载一次，共享顶点（对角线两端）也只算一次
...
fn fs(@location(0) color: vec3f) -> @location(0) vec4f {
  ...
  let cool = vec3f(0.961, 0.620, 0.043); // 琥珀
