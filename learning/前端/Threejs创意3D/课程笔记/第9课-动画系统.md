...
// 4. 动画循环中更新（delta 为这一帧到上一帧的时间差，单位是秒，来自 clock.getDelta()）
mixer.update(delta)
```
...
1. **requestAnimationFrame 循环和 gsap 动画能同时用吗？**
   > 能。推荐分工：rAF 循环做每帧逻辑（物理、LOD、渲染），GSAP 做补间动画控制，两者可以同时使用。

2. **AnimationMixer.update(delta) 的 delta 单位是什么？**
   > 秒。delta 是这一帧到上一帧的时间差（来自 clock.getDelta()），按秒数推进动画时间。

3. **如何让动画循环播放而不是只播放一次？**
   > action.setLoop(THREE.LoopRepeat, Infinity)。LoopOnce 是播放一次，LoopPingPong 是来回播放，第二个参数控制循环次数。
