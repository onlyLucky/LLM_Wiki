...
1. **GLTF 和 GLB 格式的区别是什么？**
   > GLTF 是 JSON 文本，资源分散（.gltf + .bin + 纹理多个文件），适合开发调试；
   > GLB 是二进制，单文件包含所有资源，适合生产环境。

2. **Draco 压缩的原理是什么？**
   > Google 的 3D 几何压缩库，通过量化、预测和熵编码压缩顶点数据，可减小 90%+ 文件大小（如 Damaged Helmet 4.2 MB 压到 0.8 MB）。

3. **LOD 是什么？在什么场景下使用？**
   > Level of Detail：根据物体与相机的距离自动切换不同精度的模型（近处高精度、远处低精度）。
   > 适用于大场景中的远景物体，减少渲染开销。
