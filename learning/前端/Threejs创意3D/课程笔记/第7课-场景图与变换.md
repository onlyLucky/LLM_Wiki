# 第7课-场景图与变换

> 日期：2026-07-24
> 状态：已完成
> 评分：9.8/10

---

## 📚 课程内容

### 学习目标
1. 理解场景图的树形结构
2. 掌握父子关系与变换继承
3. 区分局部坐标系 vs 世界坐标系
4. 学会 Object3D 分组管理
5. 掌握 scene.traverse() 遍历
6. 学会 AxesHelper / GridHelper 调试

---

## 🎯 核心概念

### 1. 场景图的树形结构

**定义**：Three.js 中所有物体组织成一棵树，scene 是根节点

```
scene (根节点)
├── sun (太阳)
│   ├── earth (地球)
│   │   └── moon (月球)
│   └── helper (辅助线)
└── lights (灯光)
    ├── ambientLight
    └── directionalLight
```

**关键**：子物体的变换（位置/旋转/缩放）相对于父物体

### 2. 父子关系与变换继承

**核心规则**：
- 子物体的位置是相对于父物体的
- 父物体旋转时，子物体跟着旋转
- 子物体的世界坐标 = 父物体的世界变换 × 子物体的局部变换

**示例**：
```typescript
// 地球距离太阳 5 个单位
earth.position.x = 5

// 月球距离地球 1.5 个单位（不是距离太阳 6.5）
moon.position.x = 1.5

// 构建父子关系
sun.add(earth)   // 地球是太阳的子物体
earth.add(moon)  // 月球是地球的子物体

// 当太阳旋转时，地球和月球一起绕太阳转
// 当地球旋转时，月球绕地球转
```

### 3. 旋转方向判断（右手定则）

**核心规则**：Three.js 使用右手坐标系，旋转方向用右手定则判断

**右手定则**：
- 拇指指向轴的正方向（如 +Y 轴向上）
- 四指弯曲的方向就是正旋转方向
- 正 rotation.y = 逆时针（从上往下看）

**实际应用**：
```typescript
// 地球绕太阳公转
sun.rotation.y += speed * delta  // 逆时针公转（从上往下看）

// 如果想要顺时针：
sun.rotation.y -= speed * delta  // 负值 = 顺时针
```

**常见误区**：
- ❌ 以为 rotation.y 正值 = 顺时针
- ✅ rotation.y 正值 = 逆时针（从 +Y 方向看向 -Y）

### 4. 局部坐标系 vs 世界坐标系

| 坐标系 | 说明 | 用途 |
|--------|------|------|
| 局部坐标系 | 物体自身的坐标系 | 设置物体位置、旋转 |
| 世界坐标系 | 场景的全局坐标系 | 获取物体在场景中的实际位置 |

**转换**：
```typescript
// 获取物体的世界坐标
const worldPos = new THREE.Vector3()
object.getWorldPosition(worldPos)

// 获取物体的世界四元数（旋转）
const worldQuat = new THREE.Quaternion()
object.getWorldQuaternion(worldQuat)
```

### 5. Object3D 分组管理

**Group 的作用**：把多个物体组合成一个整体

```typescript
const group = new THREE.Group()
group.add(mesh1)
group.add(mesh2)
group.add(mesh3)
scene.add(group)

// 整体移动
group.position.set(1, 2, 3)

// 整体旋转
group.rotation.y = Math.PI / 4

// 整体缩放
group.scale.set(2, 2, 2)
```

### 6. scene.traverse() 遍历

**作用**：递归遍历场景中的所有物体

```typescript
scene.traverse((child) => {
  if (child instanceof THREE.Mesh) {
    // 对每个 Mesh 执行操作
    child.castShadow = true
    child.receiveShadow = true
  }
})
```

**注意**：traverse 是深度优先遍历

### 7. AxesHelper / GridHelper 调试

**AxesHelper**：显示坐标轴
```typescript
const axesHelper = new THREE.AxesHelper(5) // 长度 5
scene.add(axesHelper)
// 红色 = X 轴，绿色 = Y 轴，蓝色 = Z 轴
```

**GridHelper**：显示网格
```typescript
const gridHelper = new THREE.GridHelper(20, 20) // 大小 20，分割 20
scene.add(gridHelper)
```

---

## 💻 代码实现要点

### 1. 太阳系父子结构

```typescript
// 创建几何体
const sun = new THREE.Mesh(sunGeo, sunMat)
const earth = new THREE.Mesh(earthGeo, earthMat)
const moon = new THREE.Mesh(moonGeo, moonMat)

// 设置相对位置
earth.position.x = 5    // 距离太阳 5
moon.position.x = 1.5   // 距离地球 1.5

// 构建父子关系
sun.add(earth)
earth.add(moon)
scene.add(sun)
```

### 2. 公转动画

```typescript
// 太阳自转 → 地球绕太阳公转
sun.rotation.y += earthSpeed * delta

// 地球自转 → 月球绕地球公转
earth.rotation.y += moonSpeed * delta
```

---

## 📝 课后作业

### 作业要求：积木小动物拼装展示

**参考案例**：[Der Baukasten](https://www.awwwards.com/sites/der-baukasten) (SOTD 7.22/10)

**目标**：用基本几何体拼装积木小动物，对比 OBJ 模型，展示父子关系和变换继承

**考察因素（100分）**：

| 项目 | 分值 | 说明 |
|------|------|------|
| 父子关系实现 | 30分 | 动物部件的正确父子结构 |
| 变换继承 | 25分 | 父部件带动子部件运动 |
| 散开/组合动画 | 20分 | 部件分散展示与组合复原 |
| 交互体验 | 15分 | 控制面板调节旋转/分散 |
| 视觉完成度 | 10分 | 材质、灯光、整体构图

**代码位置**：`src/homework/07-scene-graph/main.ts`

**已有物料**：
- OBJ 模型：`/models/baukasten/storky.obj`（左侧对照组）
- 基本几何体：BoxGeometry、ExtrudeGeometry（右侧拼装组）

### 运行方式

1. 修改 `src/main.ts` 入口指向作业代码：
   ```typescript
   // MODE = 'homework'
   import './homework/07-scene-graph/main'
   ```

2. 启动开发服务器：
   ```bash
   cd ~/Documents/code/2026/@learn/threejs_creative_3d
   pnpm dev
   ```

3. 访问 http://localhost:3300/ 查看效果

### 作业代码功能说明

| 模块 | 功能 | 对应课程知识点 |
|------|------|----------------|
| **动物拼装结构** | XYZ 轴方向理解 + 部件层级 | 局部坐标系、空间变换 |
| **变换继承** | 父部件旋转带动子部件 | 变换继承原理 |
| **散开/组合动画** | 模块分散到各个方向再复原 | Object3D 分组管理 |
| **控制面板** | 旋转/分散调节 | 交互式参数调节 |

### 评分标准

| 分数段 | 标准 |
|--------|------|
| 90-100 | 所有功能完整，动画流畅，代码结构清晰 |
| 80-89 | 核心功能完整，少量瑕疵 |
| 70-79 | 基本功能实现，缺少部分交互 |
| 60-69 | 功能不完整，但有基础实现 |
| <60 | 未完成或代码无法运行 |

---

## 🔗 相关资源

- [Three.js Scene Graph 文档](https://threejs.org/docs/#api/en/core/Object3D)
- [Three.js Helpers](https://threejs.org/docs/#api/en/helpers/AxesHelper)

---

## 复盘自测

1. **子物体的世界坐标如何计算？**
   > 子物体的世界坐标 = 父物体的世界变换 × 子物体的局部变换（子物体的位置是相对父物体的）。
   > 实际获取用 object.getWorldPosition(target)。

2. **一个物体能同时是两个父物体的子物体吗？**
   > 不能。Three.js 中所有物体组织成一棵树，scene 是根节点，树形结构里每个节点只有一个父节点。

3. **scene.traverse() 的回调参数是什么？**
   > 回调参数 child 是当前遍历到的子物体，traverse 深度优先递归遍历场景中所有物体；
   > 典型用法是判断 child instanceof THREE.Mesh 后逐个设置 castShadow 等属性。
