...
![光栅化无法处理的效果示例（软阴影、反射、AO、GI））](./images/chap7_01.png)
...
![Ray Casting 针孔相机模型](./images/chap7_03.png)
...
**局限**：只计算了直接光照，没有反射和折射（光线可以反射很多此，递归）。
...
![Whitted 1979 年渲染的 Spheres and Checkerboard](./images/chap7_04.png)
...
![Whitted-Style 光线追踪示意图](./images/chap7_05.png)
...
球面方程（中心 $\vec{c}$，半径 $R$）：球上的某一个点P到 球心 c 的距离 等于球的的 R

![光线与球面示意图](./images/chap7_07.png)

$$
(\vec{p} - \vec{c})^2 - R^2 = 0
$$

一个点既在光线上又在球上，满足两个公式，将光线方程代入球面方程：

$$
(\vec{o} + t\vec{d} - \vec{c})^2 - R^2 = 0
$$

由上$(t\vec{d} + (\vec{o}- \vec{c}))^2 - R^2 = 0$ ，进行展开$t\vec{d^2} + 2(\vec{o}- \vec{c})t\vec{d} +(\vec{o}- \vec{c})^2 - R^2 = 0$，同一元二次方程 $at^2 + bt + c = 0$，得出：
...
![光线与球面示意图](./images/chap7_07_1.png)
...
![光线与平面求交示意图](./images/chap7_07_2.png)
...
![光线与三角形求交](./images/chap7_07_3.png)
...
**基本思路**：三角形在平面内，因此分两步：
1. 光线与三角形所在平面求交
2. 判断交点是否在三角形内部（奇数点在三角形内部）

#### Moller-Trumbore 算法

一种更快的直接求光线是否过三角形的方法（适当了解），能够同时给出交点参数 $t$ 和重心坐标 $(b_1, b_2)$。
...
![光线追踪——性能挑战](./images/chap7_07_4.png)

#### 包围盒 (Bounding Volume)

**核心思想**：用简单的几何体（如盒子）包围复杂物体。如果光线不与包围盒相交，则一定不与内部物体相交，可以直接跳过。

![包围盒加速原理](./images/chap7_08.png)

#### 轴对齐包围盒 (AABB)

![轴对齐包围盒](./images/chap7_09_1.png)
...
![光线与 AABB 求交的 Slab Method](./images/chap7_09.png)
...
![光线与 AABB 求交的 计算公式](./images/chap7_09_2.png)
...
上一讲内容，从讲到为什么进行光线追踪到光线递归、光线折射反弹，计算弹射点着色；再到光线和隐式表面、平面、球、三角形如何进行求交；光线不可能跟所有空间进行求交，使用到包围盒的结构，最后讲到了比较难的光线和AABB 求交。这一讲来解决怎么利用光线对盒子求交加速光线对场景求交。
...
![空间划分示例：Oct-Tree / KD-Tree / BSP-Tree](./images/chap7_11.png)
...
![KD-Tree 遍历](./images/chap7_12_2.png)

如上图便利二叉树上的数据，便利是否有光线相交，从父节点向子节点进行查找
...
2. 递归的将包围盒物体拆成两个部分。
3. 重新计算两个拆分后的包围盒
4. 当拆分后的包围盒里面有足够少的物体就可以停止拆分了。

![BVH 物体划分](./images/chap7_13_1.png)
...
> 快速选择算法：选定一个基准值，通过一次遍历把数组分成“小于等于基准”和“大于基准”两部分，使基准落到最终正确位置。进行左边和右边的快速划分， 得到最终的中间数停止递归。
...
![BVH 遍历算法](./images/chap7_13_2.png)
...
| **辐射强度 (Radiant Intensity)** | $I(\omega)$ | 单位立体角的功率（定义不同方向上亮度） $I = \frac{d\Phi}{d\omega}$ | W/sr 或 cd (candela) |
| **辐照度 (Irradiance)** | $E(x)$ | 单位面积的功率 $E = \frac{d\Phi}{dA}$ | $W/m^2$ 或 lux |
| **辐射率 (Radiance)** | $L(p, \omega)$ | 单位立体角、单位投影面积的功率 | $W/(sr \cdot m^2)$ 或 nit |

![辐射度量学各概念的关系图](./images/chap7_13.png)
...
>微分立体角（Differential Solid Angle）是立体角的无穷小量，用于描述三维空间中某个方向周围的"角度范围"。
...
![角度&立体角&微分立体角](./images/chap7_14.png)
...
![各向同性点光源](./images/chap7_14_1.png)

$$
\Phi = \int_{S^2} I \, d\omega = 4\pi I \implies I = \frac{\Phi}{4\pi}
$$

![815 流明的 LED 灯](./images/chap7_14_2.png)
...
>**辐照度（Irradiance）** 是描述**入射到表面单位面积上的辐射功率**的物理量。E表示辐射度，$\Phi$ 表示单位时间的辐射能，A表示接收的面积。
...
>**辐射率（Radiance）** 是辐射度学中**最核心的量**，表示**单位投影面积、单位立体角上的辐射功率**。它是渲染方程的基础，也是计算机图形学中描述光传输的"通用货币"。

![辐射率 ](./images/chap7_15_2.png)
...
辐照度：面积 dA 接收的总功率  
辐射度：从方向 dω 向面积 dA 接收的功率

![辐照度与辐射率的关系 ](./images/chap7_15_3.png)
...
>**微分关系（点级别）**：来自微小立体角 dω 的辐射率 Li​ ，只有其**垂直于表面的分量**（乘以 cosθ ）才对辐照度有贡献。
