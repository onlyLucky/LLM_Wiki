# Wiki Log

> Chronological record of all wiki actions. Append-only.
> Format: `## [YYYY-MM-DD] action | subject`
> Actions: ingest, update, query, lint, create, archive, delete
> When this file exceeds 500 entries, rotate: rename to log-YYYY.md, start fresh.

## [2026-06-03] create | Wiki initialized
- Domain: 个人知识库 — 日常对话、研究笔记、技术发现和工作汇总
- Structure created with SCHEMA.md, index.md, log.md
- Daily summary cron job configured (10:00 AM daily)

## [2026-06-04] create | 每日汇总 - 2026-06-03
- 文件: daily/2026-06-03.md
- 会话来源: 飞书 (1 session, 199 messages)
- 主题: Hermes 安装配置、Wiki 搭建、飞书 Markdown 渲染、多 Agent 设置
- Cron 任务 c29483bdd4ae 首次自动执行

## [2026-06-04] update | index.md 添加目录结构
- 新增目录结构树形图
- 新增标签体系说明
- 各分类添加 emoji 和注释说明

## [2026-06-05] create | 每日汇总 - 2026-06-04
- 文件: daily/2026-06-04.md
- 会话来源: 飞书 (2 sessions, 264 messages), cron (1 session, 29 messages)
- 主题: macOS 辅助功能权限、Agent Skills 橙皮书精读笔记、飞书消息无响应排查
- 总计 3 个会话，293 条消息

## [2026-06-06] create | 每日汇总 - 2026-06-05
- 文件: daily/2026-06-05.md
- 会话来源: 飞书 (6 sessions, 871 messages), cron (1 session, 35 messages)
- 主题: 读书笔记工作流优化、Hermes 知识库 Skill、橙皮书精读、Token 优化策略、看板 UI 更新
- 总计 7 个会话，906 条消息
- index.md 同步更新：补录 2026-06-04 条目，新增 2026-06-05 条目，页面计数更新为 4

## [2026-06-07] create | 每日汇总 - 2026-06-07
- 文件: daily/2026-06-07.md
- 会话来源: cron (1 session, 1 message)
- 主题: 周日休息，无用户会话
- 总计 1 个会话（仅 Cron 自动执行），0 条用户消息
- index.md 同步更新

## [2026-06-08] create | 每日汇总 - 2026-06-06（手动补录）
- 文件: daily/2026-06-06.md
- 会话来源: cron (1 session, 1 message)
- 主题: 工作日休息，无用户会话
- 总计 1 个会话（仅 Cron 自动执行），0 条用户消息
- index.md 同步更新

## [2026-06-09] create | 每日汇总 - 2026-06-08
- 文件: daily/2026-06-08.md
- 会话来源: 飞书 (1 session), cron (2 sessions)
- 主题: Playwright 自动化能力咨询、Cron 汇总任务执行
- 总计 3 个会话，56+ 条消息
- index.md 同步更新

## [2026-06-10] create | 每日汇总 - 2026-06-10
- 文件: daily/2026-06-10.md
- 会话来源: 飞书 (1 session)
- 主题: 内容创作体系搭建、Wiki内容中心创建、选题库建立、游戏开发全流程速通
- 总计 1 个用户会话，50+ 条消息（用户消息 19 条）
- index.md 同步更新

## [2026-06-14] create | 每日汇总 - 2026-06-14
- 文件: daily/2026-06-14.md
- 补建原因：cron 10:00 运行时无用户会话，实际工作发生在 12:00-17:00
- 主题：离线网站复刻工程（follow.art + shader.se + igloo.inc）
- Git 提交：10 个
- index.md 同步更新

## [2026-06-16] create | 每日汇总 - 2026-06-15
- 文件: daily/2026-06-15.md
- 会话来源: 飞书 (1 session), cron (1 session)
- 主题: 飞书任务 API 脚本工程化（token 自动续期）、离线网站存储迁移、3 个飞书任务完成
- 总计 2 个会话，~8 条用户消息
- index.md 同步更新

## [2026-06-16] create | 每日汇总 - 2026-06-16
- 文件: daily/2026-06-16.md
- 会话来源: 飞书 (1 session), cron (1 session)
- 主题: 前端部署上线流程（域名/备案/HTTPS）、域名基础知识、ICP 备案详解、CityMemory 项目冻结
- 总计 2 个会话，10 条用户消息
- index.md 同步更新

## [2026-06-23] create | 每日汇总 - 2026-06-22
- 文件: daily/2026-06-22.md
- 会话来源: 飞书 (1 session), cron (1 session)
- 主题: animejs.com 离线复刻：wget 全站爬取、本地服务器搭建、GLB 3D 模型补齐、Draco 解码器本地化
- 总计 2 个会话，3 条用户消息
- index.md 同步更新

## [2026-06-29] create | 每日汇总 - 2026-06-29
- 文件: daily/2026-06-29.md
- 会话来源: 飞书 (1 session, 100 messages), CLI (1 session, 19 messages)
- 主题: 读书笔记扩写（《奇妙的JavaScript》973行/43K字）、飞书Gateway停运3天修复、微信读书API鉴权失败
- 总计 2 个会话，119 条消息
- index.md 同步更新

## [2026-06-30] create | 每日汇总 - 2026-06-30
- 文件: daily/2026-06-30.md
- 会话来源: 飞书 (1 session, 28 messages), cron (1 session)
- 主题: Wiki Git 提交规范制定（Conventional Commits）、GAMES101 齐次坐标笔记优化推送
- 总计 2 个会话，28 条用户消息
- index.md 同步更新

## [2026-07-06] create | 每日汇总 - 2026-07-06
- 文件: daily/2026-07-06.md
- 会话来源: 飞书 (1 session, 147 messages), cron (1 session)
- 主题: GAMES101 笔记推送、AI加速学习六种方法文档、Three.js+GLSL 4周学习计划、threejs_creative_3d 项目搭建
- 总计 2 个会话，147 条用户消息
- index.md 同步更新

## [2026-07-07] create | 每日汇总 - 2026-07-07
- 文件: daily/2026-07-07.md
- 会话来源: 飞书 (1 session, 251 messages), cron (1 session)
- 主题: Three.js 课程讲义格式规范化、第1课/第2课讲义重写、学习笔记体系梳理
- 总计 2 个会话，289 条消息
- index.md 同步更新

## [2026-07-08] create | 每日汇总 - 2026-07-08
- 文件: daily/2026-07-08.md
- 会话来源: 飞书 (5 sessions, 392 messages), cron (1 session)
- 主题: 飞书群聊免@配置、Three.js 第2/3课教学、代码参数注释规范
- 总计 6 个会话，420 条消息
- index.md 同步更新

## [2026-07-10] create | 每日汇总 - 2026-07-10
- 文件: daily/2026-07-10.md
- 会话来源: 飞书 (1 session, 16 messages)
- 主题: Three.js 课程进度查询、前两课核心知识点复习
- 总计 1 个会话，16 条消息
- index.md 同步更新
